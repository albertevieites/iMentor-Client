...
const Chat = () => {
  ...
  const getAllMessages = () => {
    ...
    if (chats().data) {
      ...
      chatService.createChat(id, otherId).then((response) => {
        setChat(response.data);
        setReceiver(
          response.data.user1._id === id
            ? response.data.user2
  ...
  const addNewMessage = () => {
    if (newMessage) {
      chatService
        .createOne(`chats/newMessage/${chat._id}`, {
          content: newMessage,
          sender: id,
          receiver: otherId,
        })
  ...
  return !messages.length ? (
    <div className="chat-page ">
      ...
      <div className="chat-box">
        <div className="chat-display-box"></div>
        <div className="message-box">
          ...
          <button type="submit" onClick={addNewMessage}>
            <i className="fa fa-send" style={{ color: "blue" }}>
              submit
            </i>
          </button>
        </div>
      ...
      <div className="chat-box">
        <div className="chat-display-box">
          {messages.map((message) => {
            ...
            const daysPassed = Math.floor(
              (new Date().getTime() - messageDate.getTime()) /
                (1000 * 3600 * 24)
            );
            const messageFooter =
              daysPassed > 0
                ? `${daysPassed} days ago`
                : `At ${messageDate.getHours()}:${messageDate.getMinutes()}`;
        ...
        <div className="message-box">
          <textarea
            ...
            onKeyDown={onEnterPress}
          ></textarea>
          <button type="submit" onClick={addNewMessage} className="sendMessage">
            <img
              src="https://res.cloudinary.com/dz2hyfmhw/image/upload/v1654785964/iMentor/Vector_paufg6.png"
              alt=""
            ></img>
          </button>
        </div>
